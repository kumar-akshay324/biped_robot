...
def knee_motion(k):
	...
	data[0], data[1] = motion.right_knee_pitch(k, current_angle[2])
	...
	# Motion start
 	...
 	for i in range(max_range):
		...
		data[6]  += rate[0]
 		...
 		# Left Knee bending
 		pwm.set_pwm(channel[8], 0, data[7] + rate[1])
		data[7]  += rate[1]
		...
		data[8]  += rate[2]		 
 		...
 		# Left Hip bending
 		pwm.set_pwm(channel[9], 0, data[9] + rate[3])
		data[9]  += rate[3]
		...
		data[10]  += rate[4]
 		...
 		# Left Ankle bending
 		pwm.set_pwm(channel[7], 0, data[11] + rate[5])
		data[11]  += rate[5]		
